...
- Removed hard dependencies: `fgsea`, `magrittr`, `data.table`, `grid`.
- Moved to Suggests: `parallel`, `ggrepel`.
- Functions: `plotEnrichment2`, `theme_common_gsea` retired (`.Defunct()`). Implementation archived in `workstation/plotGSEA.R`.
- Functions: `hypergeoTestForGeneset2` simplified to deprecated wrapper around `hypergeoTestForGeneset`. `ncore` option defunct.
- Functions: `glist_enrichment` now uses `parallel::mclapply` with `requireNamespace` guard; falls back to `lapply` if `parallel` is unavailable.
- Functions: `drawVol_gg` now checks for `ggrepel` via `requireNamespace` when `top10_lab = TRUE`.

## Ver 0.3.1.9002 [2026.03.03]
- Fixed build check warnings "no visible binding for global variable"
- Fixed License file

## Ver 0.1.7.9000 [2023.07.04]
- [v0.1.7.9000] General   : Fixed R CMD check returning the 'no visible binding' note for ggplot inputs.
- [v0.1.7.9000] Functions : `ent2sym` no longer register empty strings in entrez id vector as "not entrez".


## Ver 0.1.6.9000 [2023.02.16]
...
- [v0.1.6.9000] Functions : Deprecation warning for `hypergeoTestForGeneset2`.
...
- [v0.1.6.9001] General   : Package build check and problem fixes with devtools. (Variables, function import dependency fixes)
- [v0.1.6.9002] General   : Piping fix.
- [v0.1.6.9002] Functions : `Gen_enrichment` function..
