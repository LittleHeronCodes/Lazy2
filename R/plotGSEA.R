#' plotEnrichment2
#'
#' Draw a plot of running score for GSEA enrichment plot. 
#' Function has been taken and modified from plotEnrichment function in fgsea package.
...
#' @param gseaParam gseaParam from plotEnrichment. default 1
#' @param mtitle main title of the plot.
#' @param ytitle y axis name
#' @param ticksSize ticks width for gene set bars. default 0.4
#' @param base_size font base size. default 7
#' @param line.col line colour for GSEA running score. default "green"
#' @param lwd line width for GSEA running score. default 2
#' @param ylims y-axis limits. If NULL (default), limits will be set automatically.
#' @param draw draw plot on device. Return ggplot object if set to FALSE. default TRUE
#' @param statbar show statistics tick at bottom of plot. default FALSE
...
plotEnrichment2 <- function(
	...
	if (is.null(ylims)) {
		ylims <- c(floor(min(bottoms) * 20), ceiling(max(tops) * 20))/20
	}
	
	# set y axis ticks
	if ((ylims[2] - ylims[1]) > 0.55) {
		br <- 0.2
		ln <- seq(ceiling(ylims[1] * 5)/5, floor(ylims[2] * 5)/5, br)
	} else {
		br <- 0.1
		ln <- seq(ceiling(ylims[1] * 10)/10, floor(ylims[2] * 10)/10, br)
	}

	# position for text (top if ES < 0, bottom if ES > 0)
	txt_y_pos <- ifelse(es < 0, max(c(ln, tops)), min(c(ln, bottoms)))
	...
	xbreaks <- c(seq(xlims[1], xlims[2], 5000), xlims[2])
	...
	g1 <- ggplot(toPlot, aes(x = x_values, y = y_values)) +
		...
		annotate(
			"text", 
			x = max(toPlot$x_values), y = txt_y_pos, 
			label = txt, hjust = 1, vjust = .5, size = rel(3.0)
		) +
		scale_y_continuous(breaks = ln, limits = ylims) +
		xlim(xlims[1], xlims[2]) +
		labs(y = ytitle, title = mtitle) +
		theme_common_gsea(base_size = base_size) +
		theme(
			plot.title = element_text(
				hjust = 0.5, vjust = 0.2, face = "bold", margin = unit(c(0, 0, 1.5, 0), "mm")
			),
			panel.grid.major.y = element_line(
				colour = "grey85", linetype = "dashed", size = lwd * 0.65
			),
			axis.title.y = element_text(
				face = "bold", angle = 90, margin = unit(c(0, 1.5, 0, 0), "mm"), size = rel(0.95)
			),
	...
	if (statbar) {
		...
		g1 <- g1 + 
			geom_segment(
				x = lead$x_values, xend = lead$x_values, y = 0, yend = es, 
				color = "red", linetype = 3, size = lwd * 0.25
			)
		
		g2 <- g2 + 
			theme(plot.margin = margin(0, half_line * 2.5, 0, half_line))

		g3 <- data.frame(x_values = seq_along(stats), stat = stats) |>
			ggplot(aes(x = x_values, y = 0, xend = x_values, yend = stat)) +
			geom_segment(size = ticksSize, colour = "grey35") +
			scale_x_continuous(breaks = xbreaks, limits = xlims) +
			theme_common_gsea(base_size = base_size) +
			theme(plot.margin = margin(0, half_line * 2.5, half_line, half_line))
