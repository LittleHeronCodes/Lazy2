...
readGMT <- function(gmtfile, as.df = FALSE) {
	x <- readLines(gmtfile)
	res <- strsplit(x, "\t")
	names(res) <- vapply(res, \(y) y[1], character(1))
...
writeGMT <- function(gmtfile, genelist, geneset_desc = "") {
	...
	concat <- sapply(genelist, \(v) paste(v, collapse = "\t"))
...
hypergeoTestForGeneset <- function(
	...
	if (!all(unlist(refGMT) %in% gspace)) {
		refGMT <- lapply(refGMT, \(g) intersect(g, gspace))
	}
	...
	if (length(exc) != 0) {
		if (length(exc) <= 5) {
			...
			msg <- paste(
				length(exc), "entries in refGMT had less than", minGeneSet, "genes and were excluded."
			)
		}
		warning(msg)
	...
	jacc   <- qs / sapply(refGMT, \(x) length(union(x, query)))
	gs.ratio <- paste0(qs, "/", k)
	bg.ratio <- paste0(ms, "/", N)
	int_genes <- sapply(intscts, \(x) paste(x, collapse = "/"))
	...
	pv <- with(enrRes, ifelse(int == 0, NA, pVal))
	enrRes <- enrRes |>
		dplyr::mutate(
...
#' Deprecated wrapper. Use \code{hypergeoTestForGeneset} directly.
#' @export

hypergeoTestForGeneset2 <- function(
	query, refGMT, gspace, minGeneSet = 10, ncore = NULL, ef.psc = 0
) {
	.Deprecated("hypergeoTestForGeneset")
	if (!is.null(ncore)) {
		warning("'ncore' is defunct and will be ignored.", call. = FALSE)
	}
	hypergeoTestForGeneset(query, refGMT, gspace, minGeneSet = minGeneSet, ef.psc = ef.psc)
}
